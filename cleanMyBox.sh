...
INSTALLED_KERNELS=$(dpkg --list | awk '/^ii.*linux-image-[0-9]/{print $2}' | grep -v -e "$(uname -r | sed 's/-amd64//')" -e "linux-image-amd64")

# Get the list of removed kernels (rc status)
REMOVED_KERNELS=$(dpkg --list | awk '/^rc.*linux-image-[0-9]/{print $2}')
